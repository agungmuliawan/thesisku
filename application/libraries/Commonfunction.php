...
class Commonfunction
{
	...
	*/	
	...
	public function createRadio($cboid,$cboval,$count,$cboDef)
	{
			...
			{
				$chk="";
				if($cboDef==$cboid[$i] || $cboDef==$cboval[$i])
				$chk="checked"	;
	...
	public function createCheckboxfromDB($cboTb,$cboSel,$count="",$cboDef = null)
	{
			//init modal
			$cboemp=" 
                   ";
			...
			foreach($qemp->result() as $row)
			{
				...
				$cboemp.=" 	<label style='margin-right:10px;width:150px'>
								<input type='checkbox' name=txtIsi[] class='minimal' value='".$row->id."' $chk>&nbsp;".$row->nm."&nbsp;
							</label>";
			}
	...
	public function createCheckbox($cboTb,$cboSel,$cboDef=null,$cboName="txt[]")
	{
			//init modal
			$cboemp=" 
                   ";
			...
			foreach($cboTb as $i => $row)
			{
				$chk="";
				if($cboDef <> null)
				foreach($cboDef as $def)
				{
					if($def == $row || $def == $cboSel[$i] )
						$chk="checked"	;
					
				}
			
				$cboemp.=" 	<label style='margin-right:10px;width:auto'>
								<input type='checkbox' name=$cboName class='minimal' value='".$row."' $chk>&nbsp;".$cboSel[$i]."&nbsp;
							</label>";
			}
